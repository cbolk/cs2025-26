...
First solution for filtering (seen together) 

```Python
# 2. filter
# all information is in the result table and in the new dataframe
# inner or right join leads to the same result
# inner can be omitted
firstlabres = pd.merge(dfout, dfpiddate, on=['PID', 'VisitDate'], how='inner')
...
lab_wide["Risk"] = HBA1C_MULT * lab_wide["HbA1c"] + LDL_MULT * lab_wide["LDL"] 
 + WEIGHT_MULT * lab_wide["Weight_kg"] + BP_MULT * lab_wide["Systolic_BP"]
