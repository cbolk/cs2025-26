...
def is_anagram(w1, w2):
    if len(w1) != len(w2):
        return False
    counters1 = [0] * ASCIICHARS_SIZE
    counters2 = [0] * ASCIICHARS_SIZE
    for c in w1:
        counters1[ord(c)] += 1
    for c in w2:
        counters2[ord(c)] += 1
...
def find_anagrams(word, dicFilename):
    ...
    try:
        ...
        for line in fin:
            word_dic = line.strip().lower()
            if word != word_dic and is_anagram(word, word_dic):
                anagrams.append(word_dic)
...
::: exefatti
Write Python functions to deal with TicTacToe Leaderboard by updating a file `leaderboard.csv` that contains the number of wins for each player in the format:
```
player,wins
Alice,2
Bob,1
...
[file](../codice/46.tictactoeleadership.py)
...
A file named `patients.txt ` contains information about patients in the following format:
```
name,weight,height
Alice,68,1.65
Bob,85,1.80
...
def compute_bmi_from_file(filename):
    """
    Reads a file with patient data and computes BMI for each patient.
...
[file](../codice/90.file.patients.py)
...
* Updates the board with the move, and prints back whether move is valid, invalid or whether there’s a player who wins the game.
...
[file](../codice/90.tictactoemove.py)
